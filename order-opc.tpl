...
{else}
	{if $productNumber}
		...
		<!-- END Carrier -->
		<div id="opcbuttonandterms">
			<button type="submit" name="submitAccount" id="submitAccount" class="btn btn-default button button-medium"><span>{l s='Save customer information and accept Terms of Service'}<i class="icon-chevron-right right"></i></span></button>
			{*
			<div style="display: none;" id="opc_account_saved" class="alert alert-success">
				{l s='Customer information saved successfully and Terms of Service agreed on'}
			</div>
			*}
			<div><a href="{$link_conditions|escape:'html':'UTF-8'}" class="iframe" rel="nofollow" title="{l s='(Read the Terms of Service)'}">{l s='(Read the Terms of Service)'}</a></div>
		</div>
		<!-- Payment -->
