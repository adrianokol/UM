$(document).ready(function(){
	...
	if ((typeof isLogged == 'undefined' || !isLogged) || (typeof isGuest !== 'undefined' && isGuest))
	{
		...
		$(document).on('click', '#submitAccount, #submitGuestAccount', function(e){
			...
			$('#customer_lastname').val($('#firstname').val());
			$('#customer_firstname').val($('#lastname').val());
			...
			$.ajax({
				...
				success: function(jsonData)
				{
					...
					else
					{
						...
						updatePaymentMethodsDisplay(1);
						//scroll to payment
						$('html, body').animate({
							scrollTop: $("#opc_payment_methods").offset().top - 200
						}, 2000);
					}
...
function updatePaymentMethodsDisplay(checked)
{
	//checked = 1;
	/*var checked = '';
